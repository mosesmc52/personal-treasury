...
def _render(title, summary, previous, period_label):
    ...
    lines += [f"Income:                {_money(summary['total_income'])}", f"Net cash flow:         {_money(summary['net_cash_flow'])}", f"Transactions:          {summary['transaction_count']}", f"Average spending/day:  {_money(summary['average_daily_spending'])}", "", "SPENDING BY CATEGORY"]
    ...
    lines += ["", "CASH FLOW", f"Income:                {_money(summary['total_income'])}", f"Spending:             -{_money(summary['total_spending'])}", f"Net cash flow:          {_money(summary['net_cash_flow'])}"]
