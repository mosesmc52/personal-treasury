...
def _validate_and_build_rule(key, values):
    ...
    if rule_type not in RULE_TYPES:
        raise ValueError(
            f"accounts.{key}.type must be one of: {', '.join(sorted(RULE_TYPES))}"
        )
    ...
    target = (
        _decimal(values["target"], f"accounts.{key}.target")
        if "target" in values
        else None
    )
    percentage = (
        _decimal(values["percentage"], f"accounts.{key}.percentage")
        if "percentage" in values
        else None
    )
    maximum = (
        _decimal(
            values["maximum_allocation_per_run"],
            f"accounts.{key}.maximum_allocation_per_run",
        )
        if "maximum_allocation_per_run" in values
        else None
    )
    monthly_amount = (
        _decimal(values["monthly_amount"], f"accounts.{key}.monthly_amount")
        if "monthly_amount" in values
        else None
    )
    ...
    if maximum is not None and maximum < 0:
        raise ValueError(
            f"accounts.{key}.maximum_allocation_per_run cannot be negative"
        )
...
def load_allocation_config(path="config/allocation.yaml"):
    ...
    minimum = _decimal(
        raw_settings.get("minimum_allocation", 0), "settings.minimum_allocation"
    )
    round_to = _decimal(raw_settings.get("round_to", "0.01"), "settings.round_to")
    maximum_total = (
        _decimal(
            raw_settings["maximum_total_allocation"],
            "settings.maximum_total_allocation",
        )
        if "maximum_total_allocation" in raw_settings
        else None
    )
    if minimum < 0:
    ...
    rules = tuple(
        _validate_and_build_rule(key, values) for key, values in raw_accounts.items()
    )
    ...
    for priority in sorted({rule.priority for rule in rules}):
        group = [
            rule
            for rule in rules
            if rule.priority == priority and rule.type == "percentage"
        ]
        if group:
            total = sum((rule.percentage for rule in group), Decimal("0"))
            if abs(total - Decimal("1")) > Decimal("0.000001"):
                raise ValueError(
                    f"percentage rules at priority {priority} must total 1.0; got {total}"
                )
