...
def is_pending(t):
    return bool(t.get("pending", False))


def _text(t):
    return f"{t.get('name','')} {t.get('merchant_name','')}".lower()


def is_internal_transfer(t):
    return any(x in _text(t) for x in ("transfer", "zelle", "venmo")) and not is_refund(
        t
    )


def is_credit_card_payment(t):
    return (
        "credit card" in _text(t) or "card payment" in _text(t) or "autopay" in _text(t)
    )
...
def is_income(t):
    return (
        t.get("amount", 0) < 0
        and not is_internal_transfer(t)
        and not is_refund(t)
        and any(
            x in _text(t)
            for x in ("payroll", "salary", "direct deposit", "interest", "income")
        )
    )


def is_refund(t):
    return t.get("amount", 0) < 0 and any(
        x in _text(t) for x in ("refund", "reversal", "credit")
    )
...
def is_spending(t):
    return (
        ...
        and not is_internal_transfer(t)
        and not is_credit_card_payment(t)
    )
...
def category(t):
    ...
    }.get(raw, raw.replace("_", " ").title() if raw else "Other")


def get_spending_summary(transactions, start_date, end_date):
    start_date, end_date = str(start_date), str(end_date)
    spending, income = [], []
    cash_flow_by_account = defaultdict(
        lambda: {
            "inflows": 0.0,
            "outflows": 0.0,
            "net_cash_flow": 0.0,
    ...
    for t in transactions:
        if not (t.get("date") and start_date <= t["date"] <= end_date):
            continue
        if not is_pending(t):
        ...
        if is_income(t):
            income.append(t)
        if is_spending(t) or is_refund(t):
            spending.append(t)
    ...
    return {
        ...
        "total_account_net_cash_flow": total_inflows - total_outflows,
        ...
        "spending_by_merchant": dict(by_merchant),
        "largest_expenses": [
            {
                "date": t["date"],
                "merchant": merchant(t),
        ...
        "average_daily_spending": total_spending / days if days else 0,
        "savings_rate": (
            (total_income - total_spending) / total_income if total_income > 0 else None
        ),
        "cash_flow_by_account": cash_flow_by_account,
    }
