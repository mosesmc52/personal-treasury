...
def create_plaid_client(settings: Settings | None = None):
    ...
    missing = [name for name, value in (("PLAID_CLIENT_ID", settings.plaid_client_id), ("PLAID_SECRET", settings.plaid_secret), ("PLAID_ACCESS_TOKEN", settings.plaid_access_token)) if not value]
    if missing:
    ...
    return PlaidApi(ApiClient(configuration)), settings.plaid_access_token
