...
def _run_weekly(transactions, as_of):
    ...
    subject = (
        f"Personal Treasury — Weekly Report — {period_start:%b %-d}–{period_end:%-d}"
    )
    print(content)
...
def _run_allocation(config_path, state_path, as_of, email=False):
    ...
    if (
        ...
        or "available_cash" not in state
        or not isinstance(state.get("balances"), dict)
    ):
        raise ValueError(
            "Allocation state must contain available_cash and a balances mapping"
        )
    ...
    if email:
        return _email(
            f"Personal Treasury — Allocation Recommendation — {as_of:%b %-d, %Y}",
            content,
        )
    return 0
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Personal Treasury scheduled financial reports"
    )
    parser.add_argument(
        "command", choices=("sync", "daily", "weekly", "monthly", "allocate")
    )
    parser.add_argument(
        "--as-of",
        type=date.fromisoformat,
        help="Use this date instead of today (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--weekly-day",
        default="sunday",
        choices=tuple(WEEKDAYS),
        help="Daily mode report day (default: sunday)",
    )
    parser.add_argument(
        "--email", action="store_true", help="Send the generated report by email"
    )
    parser.add_argument(
        "--config", default="config/allocation.yaml", help="Allocation policy YAML path"
    )
    parser.add_argument(
        "--state",
        default="data/allocation_state.json",
        help="Allocation financial-state JSON path",
    )
    parser.add_argument(
        "--income",
        type=float,
        default=None,
        help="Net paycheck amount; sets available_cash to this amount",
    )
    ...
    try:
        ...
        if as_of.day == last_day:
            ...
            logging.info(
                "Monthly report skipped: today is not the last day of the month."
            )
        return status
    except Exception as exc:
        logging.error("%s", exc)
